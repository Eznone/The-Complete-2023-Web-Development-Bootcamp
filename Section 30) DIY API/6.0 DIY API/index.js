...
app.patch("/jokes/:id", (req, res) => {
  ...
  if (jokes.findIndex((joke) => joke.id === id) !== -1){
    if (request.type || request.text){
      ...
      const newJoke = {
      id: existingJoke.id,
      jokeText: request.text || existingJoke.jokeText,
      jokeType: request.type || existingJoke.jokeType,
    }
    jokes[existingIndex] = newJoke;
    console.log(newJoke);
    res.json(newJoke);  
    } else {
      console.log("No text or type recieved");
      res.json({error: "No text or type recieved"});
    }
  } else {
...
app.delete("/all", (req, res) => {
  ...
  if (userKey === masterKey){
    ...
    console.log(jokes);
    res.json({success: "Deleted all Jokes successfully"});
  } else {
